...
big_datasets = ['FB15K', 'analogy', 'MCNet']
...
if args.finetune:
    if args.ckpt is not None:
        if args.model == 'ComplEx':
            ...
            model.load_state_dict(torch.load(args.ckpt))
    model.finetune = True
    model.analogy_entids = [int(ent) for ent in read_txt('data/MCNet/analogy_ent_id')]
    model.analogy_relids = [int(rel) for rel in read_txt('data/MCNet/analogy_rel_id')]
...
if not args.finetune:
    ...
    for e in range(args.max_epochs):
        ...
        if (e + 1) % args.valid == 0:
            ...
            if test['MRR'] > best_test_mrr:
                ...
                torch.save(model.state_dict(), 'checkpoint/pt_best_model_analogy_MCNet_30.pth')
    ...
    for e in range(args.max_epochs):
        ...
        if (e + 1) % args.valid == 0:
            ...
            if test['MRR'] > best_test_mrr:
                best_test_mrr = test['MRR']
                torch.save(model.state_dict(), f'checkpoint/ft_best_model_{args.model}_{args.learning_rate}_300.pth')
