...
from .processor_struct import KGProcessor, get_dataset
...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features, return_tensors=None):
        ...
        features_keys = {}
        if self.pretrain:
            struct_head_id = [feature.pop("struct_head_id") for feature in features]
            struct_rel_id = [feature.pop("struct_rel_id") for feature in features]
            struct_tail_id = [feature.pop("struct_tail_id") for feature in features]
            head_token_idx = [feature.pop("head_token_idx") for feature in features]
            tail_token_idx = [feature.pop("tail_token_idx") for feature in features]
            struct_a_head_id = None
            a_head_token_idx = None
        else:
            struct_head_id = [feature.pop("struct_q_head_id") for feature in features] # q_head 对应 head
            struct_rel_id = [feature.pop("struct_rel_id") for feature in features]
            struct_tail_id = [feature.pop("struct_q_tail_id") for feature in features] # q_tail 对应 tail
            struct_a_head_id = [feature.pop("struct_a_head_id") for feature in features] # a_head 是新增的
            head_token_idx = [feature.pop("q_head_token_idx") for feature in features]
            tail_token_idx = [feature.pop("q_tail_token_idx") for feature in features]
            a_head_token_idx = [feature.pop("a_head_token_idx") for feature in features] # a_head_idx 也是新增的
        ...
        q_tail_idx = [feature.pop("q_tail_idx") for feature in features] if "q_tail_idx" in features[0].keys() else None
        ...
        features['struct_head_id'] = torch.tensor(struct_head_id, dtype=torch.long)
        features['struct_rel_id'] = torch.tensor(struct_rel_id, dtype=torch.long)
        features['struct_tail_id'] = torch.tensor(struct_tail_id, dtype=torch.long)
        
        features['head_token_idx'] = torch.tensor(head_token_idx, dtype=torch.long)
        features['tail_token_idx'] = torch.tensor(tail_token_idx, dtype=torch.long)
        
        # 为微调任务新增的字段
        if struct_a_head_id is not None:
            features['struct_a_head_id'] = torch.tensor(struct_a_head_id, dtype=torch.long)
        if a_head_token_idx is not None:
            features['a_head_token_idx'] = torch.tensor(a_head_token_idx, dtype=torch.long)
        ...
        if q_tail_idx:
            features['q_tail_idx'] = torch.tensor(q_tail_idx)
...
class KGC(BaseDataModule):
    def __init__(self, args, model) -> None:
        ...
        # +++ 新增代码：创建并存储名称映射 +++
        # 使用辅助方法获取名称列表
        entity_names = self._get_entity_names(os.path.join(self.args.pretrain_path, "entity2text.txt"))
        relation_names = self._get_relation_names(self.args.pretrain_path)
        
        # 创建 ID -> Name 的反向映射
        self.id2entity = {i: name for i, name in enumerate(entity_names)}
        self.id2relation = {i: name for i, name in enumerate(relation_names)}
        # ++++++++++++++++++++++++++++++
        ...
        print(f"Loading structural mappings from: {args.structural_data_dir}")
        self.struct_entity2id = self._load_struct_mapping(os.path.join(args.structural_data_dir, "entity2id.txt"))
        self.struct_relation2id = self._load_struct_mapping(os.path.join(args.structural_data_dir, "relation2id.txt"))
        # change, add structural embedding
        #self.processor = KGProcessor(self.tokenizer, args)
        
        # +++ 新增：创建反向映射字典 +++
        self.struct_id2entity = {v: k for k, v in self.struct_entity2id.items()}
        self.processor = KGProcessor(self.tokenizer, args, self.struct_entity2id, self.struct_relation2id)
        ...
        # change, add entity2struct_features
        ...
        # change, add entity2struct_features
        # struct_path = os.path.join(self.args.pretrain_path, 'entity_struct_emb.npy')
        # entity2struct_features = torch.from_numpy(np.load(struct_path))    
    ...
    def get_config(self):
        ...
        for k, v in self.__dict__.items():
            if "st" in k or "ed" in k or 'analogy' in k:
                d.update({k:v})
        # 在循环之后，显式地添加我们需要的反向映射字典
        if hasattr(self, 'struct_id2entity'):
            d['struct_id2entity'] = self.struct_id2entity
        
        # +++ 新增代码：将新的映射字典添加到配置中 +++
        if hasattr(self, 'id2entity'):
            d['id2entity'] = self.id2entity
        if hasattr(self, 'id2relation'):
            d['id2relation'] = self.id2relation
        # +++++++++++++++++++++++++++++++++++++++
    ...
    def _get_entity_names(self, file_path):
        """从文件中读取实体名称列表"""
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            entities = [line.strip().split("\t")[0] for line in lines]
        return entities

    def _get_relation_names(self, data_dir):
        """从文件中读取关系名称列表"""
        with open(os.path.join(data_dir, "relation2text.txt"), 'r') as f:
            lines = f.readlines()
            relations = [line.strip().split('\t')[0] for line in lines]
        return relations
    
    def _load_struct_mapping(self, file_path):
        mapping = {}
        with open(file_path, 'r', encoding='utf-8') as f:
            # Relphormer的ID文件可能没有总数行，直接读取
            for line in f:
                # 格式通常是 "实体名称<tab>ID"
                parts = line.strip().split('\t')
                if len(parts) == 2:
                    item, item_id = parts
                    mapping[item] = int(item_id)
        return mapping
